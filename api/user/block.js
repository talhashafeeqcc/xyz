...
async function view(req, res, token) {
  ...
  mailer({
    to: user.email,
    subject: `This account has been blocked on ${req.headers.host}${process.env.DIR || ''}`,
    text: `You are will no longer be able to log on to ${req.headers.host.includes('localhost') && 'http' || 'https'}://${req.headers.host}${process.env.DIR || ''}`
  });
