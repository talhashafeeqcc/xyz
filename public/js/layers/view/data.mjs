export default _xyz => {
  ...
  function panel(layer) {
    ...
    if (!layer.dataviews || !_xyz.dataview.tabview.node) return;
      ...
      Object.entries(layer.dataviews).forEach(dataview => {
        ...
        <input
          ...
          onchange=${e => {
            ...
            if (dataview[1].display) return _xyz.dataview.tabview.add(dataview[1]);
            dataview[1].remove();
          }}>
        </input>
        <div></div><span>${dataview[1].title || dataview[0]}`);

        if (dataview[1].display && layer.display) _xyz.dataview.tabview.add(dataview[1]);
      });
